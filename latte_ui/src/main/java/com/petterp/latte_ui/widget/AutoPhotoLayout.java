...
public final class AutoPhotoLayout extends LinearLayoutCompat {
    ...
    public AutoPhotoLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        //从xml文件中获取需要的参数
        final TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.camera_from_layout);
        //默认值为1
        mMaxNum = typedArray.getInt(R.styleable.camera_from_layout_max_count, 1);
        //如果什么都没传，默认显示3个
        mMaxLineNum = typedArray.getInt(R.styleable.camera_from_layout_line_count, 3);
        //没有图片，默认无间隙
        mImageMargin = typedArray.getInt(R.styleable.camera_from_layout_item_margin, 0);
        //默认20大小item
        mIconSize = typedArray.getDimension(R.styleable.camera_from_layout_icon_size, 20);
        //回收资源
        typedArray.recycle();
    }
    ...
    /**
     * 确定子控件位置
     * 简单来说，将子控件添加到容器中，添加时判断是否超过相应的设置，否则添加到新的一行，最后遍历添加过的，设置相应子view的位置等。
     *
     * @param changed
     * @param l
     * @param t
